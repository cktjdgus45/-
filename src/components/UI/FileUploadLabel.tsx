...
const FileUploadLabel = ({ file, dragging, handleDrag, handleDragOver, handleDrop }) => {
    return (
        <label aria-label='업로드할 파일을 끌어다 놓아주세요.' className={`w-full h-60 flex flex-col items-center  justify-center ${!file && 'border-2 border-main-color border-dashed'}`} htmlFor="input-upload" onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDragOver} onDrop={handleDrop}>
            {dragging && (
